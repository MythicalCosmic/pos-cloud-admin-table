...
/* Ranks the full movers list so the Absolute / Percent toggle reorders it. */
...
<template>
  <ChartCard
    :eyebrow="t('Top movers')"
    :title="t('Biggest gainers & losers')"
  >
    <template #actions>
      <div class="seg">
        <button
    ...
    <div class="movers">
      ...
      <div class="movers__col">
        ...
        >
          ...
          <div class="mrow__bar">
            <span
              class="mrow__fill down"
              :style="{ width: barPct(m) }"
            />
          </div>
          <div class="mrow__val down">
            −{{ abbrUZS(Math.abs(m.delta)) }}
          </div>
        </div>
        <div
          v-if="!losers.length"
...
<style scoped>
.movers { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: var(--sp-4); }
.movers__col { min-width: 0; }
...
.mrow__name { flex: 1; min-width: 0; font-size: 13px; font-weight: 500; color: var(--text); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
