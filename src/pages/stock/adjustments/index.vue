...
async function loadLookup<T = any>(
  ...
  try {
    ...
    if (!list.length && Array.isArray(d))
      list = d as T[]
...
function validateAdjust(): boolean {
  ...
  if (!adjust.value.stock_item_id)
    e.stock_item_id = t('required_field')
  else if (itemTrackingLoading.value)
    e.stock_item_id = t('stock_adjust_item_verification_pending')
  else if (itemTrackingError.value)
    e.stock_item_id = itemTrackingError.value
  else if (selectedItemTracksBatches.value)
    e.stock_item_id = t('stock_adjust_batch_tracked_blocked')
  if (!adjust.value.location_id)
    e.location_id = t('required_field')
  if (!allowedAdjustmentTypes.has(adjust.value.movement_type))
    e.movement_type = t('required_field')
  if (!normalizedAdjustmentQuantity(adjust.value.quantity)) { e.quantity = t('stock_adjust_quantity_invalid') }
  else if (isOutgoingAdjustment.value) {
    if (availabilityLoading.value)
      e.quantity = t('stock_adjust_availability_check_pending')
    else if (availabilityError.value)
      e.quantity = availabilityError.value
...
function validateCode(): boolean {
  const e: Record<string, string> = {}
  if (codeMode.value === 'create' && !codeForm.value.code.trim())
    e.code = t('required_field')
  if (!codeForm.value.name.trim())
    e.name = t('required_field')
  codeErrors.value = e
  return Object.keys(e).length === 0
}
...
<template>
  <WorkspacePage class="page">
    ...
    >
      ...
      >
        ...
        <template #cell.movement_type="{ row }">
          <Badge
            :tone="(MOVEMENT_TONE[row.movement_type] ?? 'neutral') as any"
            dot
          >
    ...
    <div class="card">
      ...
      <WorkspaceToolbar class="toolbar wrap">
        <div
    ...
    >
      <div style="display: grid; gap: var(--sp-3);">
        ...
        <label
          class="row"
          style="gap: 10px; cursor: pointer; padding: 4px 0;"
        >
