...
test('login honors reduced motion and keeps theme controls synchronized', async ({ page }) => {
  ...
  expect(await page.locator('.login-scene__hub').evaluate(el => getComputedStyle(el).animationName)).toBe('none')
...
test('login decorative motion can be paused without affecting the form', async ({ page }) => {
  ...
  expect(await page.locator('.login-scene__hub').evaluate(el => getComputedStyle(el).animationPlayState)).toBe('paused')
  await fillLogin(page)
  await page.getByRole('button', { name: 'Play illustration animation' }).click()
  expect(await page.locator('.login-scene__hub').evaluate(el => getComputedStyle(el).animationPlayState)).toBe('running')
  await expect(page.locator('#login-password')).toHaveValue(password)
})
